...
async function init() {
    ...
    if (!IS_WEB && IS_DISCORD_DESKTOP && !IS_UPDATER_DISABLED) {
        let healthy = false;
        for (let attempt = 0; attempt < 3 && !healthy; attempt++) {
            try {
                healthy = await VencordNative.updater.markReleaseHealthy();
            } catch { }
            if (!healthy && attempt < 2)
                await new Promise(resolve => setTimeout(resolve, 500));
        }
        if (!healthy)
            UpdateLogger.error("The active release could not be marked healthy.");
    }
