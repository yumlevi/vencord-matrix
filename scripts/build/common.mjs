...
export const UPDATE_RSA_MODULUS = process.env.VENCORD_UPDATE_RSA_MODULUS ?? "";
export const UPDATE_RSA_EXPONENT = process.env.VENCORD_UPDATE_RSA_EXPONENT ?? "";
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            ...
            for (const dir of pluginDirs) {
                ...
                for (const file of files) {
                    ...
                    // MatrixBridge depends on a sandboxed Electron worker and
                    // native IPC assets. Keep it out of browser/extension builds
                    // even if synced settings say the plugin was enabled.
                    const target = dir === "plugins" && fileName === "matrixBridge"
                        ? "desktop"
                        : getPluginTarget(fileName);
